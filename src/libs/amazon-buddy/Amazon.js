...
export class AmazonScraper {
    ...
    get getReferer() {
        if (Array.isArray(this.referer)) {
            return this.referer[
                Math.floor(Math.random() * this.referer.length)
            ];
        }
    ...
    async startScraper() {
        if (this.scrapeType === "products") {
            ...
            if (this.number > CONST.limit.product) {
                throw new Error(
                    `Wow.... slow down cowboy. Maximum you can get is ${CONST.limit.product} products`
                );
            }
        ...
        if (this.scrapeType === "reviews") {
            ...
            if (this.number > CONST.limit.reviews) {
                throw new Error(
                    `Wow.... slow down cowboy. Maximum you can get is ${CONST.limit.reviews} reviews`
                );
            }
        }
        if (this.scrapeType === "asin" && !this.asin) {
            throw new Error("ASIN is missing");
        }
            ...
            forEachLimit(
                ...
                async (item) => {
                    const body = await this.buildRequest(
                        this.bulk ? item : this.searchPage
                    );
                    ...
                    if (this.scrapeType === "products") {
                        const totalResultCount = body.match(
                            /"totalResultCount":\w+(.[0-9])/gm
                        );

                        if (totalResultCount) {
                            this.totalProducts =
                                totalResultCount[0].split(
                                    'totalResultCount":'
                                )[1];
                        }
            ...
            switch (this.fileType) {
                ...
                case "csv":
                    await fromCallback((cb) =>
                        writeFile(
                            `${this.fileName}.csv`,
                            this.jsonToCsv.parse(this.collector),
                            cb
                        )
                    );
                    break;
     ...
     */
    sortAndFilterResult() {
        if (this.scrapeType === "reviews" && this.sort) {
            this.collector.sort((a, b) => b.rating - a.rating);
        }
        if (this.scrapeType === "products") {
            this.collector.sort(
                (a, b) =>
                    a.position.global_position - b.position.global_position
            );
            this.collector.forEach((item, index) => {
                item.position.global_position = index++;
            });
            ...
            if (this.discount) {
                this.collector = this.collector.filter(
        ...
        if (this.scrapeType === "products" || this.scrapeType === "reviews") {
            this.collector = this.collector.filter((item) =>
                this.validateRating(item)
            );
        }
    }
        ...
        switch (this.scrapeType) {
            ...
            case "reviews":
                return `product-reviews/${
                    this.asin
                }/ref=cm_cr_arp_d_viewopt_srt?formatType=${
                    CONST.reviewFilter.formatType[this.reviewFilter.formatType]
                }&sortBy=${
                    CONST.reviewFilter.sortBy[this.reviewFilter.sortBy]
                        ? CONST.reviewFilter.sortBy[this.reviewFilter.sortBy]
                        : ""
                }${
                    this.reviewFilter.verifiedPurchaseOnly
                        ? "&reviewerType=avp_only_reviews"
                        : ""
                }${
                    this.reviewFilter.filterByStar
                        ? `&filterByStar=${
                              CONST.reviewFilter.filterByStar[
                                  this.reviewFilter.filterByStar
                              ]
                          }`
                        : ""
    ...
    grabReviews(body) {
         ...
         */
        this.reviewMetadata.total_reviews = parseInt(
            $(".averageStarRatingNumerical").text(),
            10
        );

        const reviewsListContainer = $("#cm_cr-review_list");
        if (!reviewsListContainer.length) {
            return false;
        }

        const reviewsList = reviewsListContainer[0].children;
        const scrapingResult = {};
        for (const reviewsItem of reviewsList) {
            const totalInResult =
                Object.keys(scrapingResult).length + this.collector.length;
            if (totalInResult >= this.number && this.bulk) {
                break;
            }
            const reviewId = reviewsItem.attribs.id;
            if (!reviewId) {
                continue;
            }
        ...
        for (const key in scrapingResult) {
            ...
            try {
                ...
                if (scrapingResult[key].review_data) {
                    scrapingResult[key].date = this.geo.review_date(
                        scrapingResult[key].review_data
                    );
                }
            } catch (error) {
                continue;
        ...
        for (const key in scrapingResult) {
            const search = $(
                `#${key} div.a-row.a-spacing-mini.review-data.review-format-strip > a`
            );
        ...
        for (const key in scrapingResult) {
            ...
            try {
                scrapingResult[key].rating = parseFloat(
                    search[0].children[0].children[0].data.split(" ")[0]
                );
            } catch (error) {
                continue;
            }
         ...
         */
        for (const key in scrapingResult) {
            const search = $(`#${key} [data-hook="review-image-tile"]`);
            scrapingResult[key].photos = [];

            try {
                search.each((index, element) => {
                    scrapingResult[key].photos.push(
                        $(element).attr("src").replace("_SY88", "_SL1600")
                    );
                });
            } catch (error) {
                continue;
    ...
    extractProductFeatures($) {
        ...
        try {
            ...
            for (const item in featureList) {
                if (featureList[item].children[0].data)
                    featureBullets.push(featureList[item].children[0].data);
            }
        } catch {
        ...
        try {
            const featureListExpanded = $("#feature-bullets > div > div")[0]
                .children[0].children;
            for (const item of featureListExpanded) {
                if (item.children[0].children[0].data)
                    featureBullets.push(item.children[0].children[0].data);
            }
        } catch {
            // continue regardless of error
        }
    ...
    extractProductInfromation($) {
        ...
        try {
            ...
            for (const item in bestSalesSection) {
                const rank = $(bestSalesSection[item].children[0]).text();
                const category = $(bestSalesSection[item].children[1]).text();
                const link =
                    bestSalesSection[item].children[1].children[1].attribs.href;
                if (category && rank) {
                    const bestSeller = this.geo.best_seller(
                        `${rank} ${category}`
                    );
                    if (bestSeller) {
                        bestsellers_rank.push({
                            ...bestSeller,
                            link: `${this.mainHost}${link}`,
                        });
                    }
                }
            }
        ...
        try {
            const bestSalesSection = $(
                `#detailBulletsWrapper_feature_div > ul:nth-child(5) > li > span`
            )[0];
            bestSalesSection.children.forEach((item) => {
                if (item.type === "text" && item.data != "") {
                    ...
                    if (bestSeller) {
                        bestsellers_rank.push({
                            ...bestSeller,
                            link: `${this.mainHost}${item.next.attribs.href}`,
                        });
                    }
                }
                if (
                    ...
                    item.children.forEach((rankItem) => {
                        ...
                        const category = $(
                            rankItem.children[0].children[1]
                        ).text();
                        const link =
                            rankItem.children[0].children[1].attribs.href;
                        if (category && rank) {
                            const bestSeller = this.geo.best_seller(
                                `${rank} ${category}`
                            );
        ...
        try {
            for (const id of this.geo.product_information.id) {
                const informationSection = $(id);
                if (informationSection[0]) {
                    const len = informationSection[0].children.length;
                    const arr =
                        len === 1
                            ? informationSection[0].children[0].children
                            : informationSection[0].children;
                    ...
                    for (const item of arr) {
                        ...
                        if (len === 1) {
                            ...
                            try {
                                key =
                                    item.children[0].children[0].children[0].data.replace(
                                        ":",
                                        ""
                                    );
                                value =
                                    item.children[0].children[1].children[0]
                                        .data;
                            } catch {
                                // continue regardless of error
                            }
                        ...
                        if (fieldToExtract) {
                            ...
                            if (fieldToExtract && value) {
                                if (
                                    fieldToExtract.key === "dimensions" &&
                                    value.indexOf("; ") > -1
                                ) {
                                    const splitDimensions = value.split("; ");
                                    if (
                                        !product_information.weight &&
                                        splitDimensions[1]
                                    ) {
                                        product_information.weight =
                                            splitDimensions[1];
        ...
        if (product_information.available_from) {
            ...
            try {
                product_information.available_from_utc = new Date(
                    product_information.available_from
                ).toISOString();
                product_information.available_for_months = Math.ceil(
                    duration.asMonths()
                );
                product_information.available_for_days = Math.ceil(
                    duration.asDays()
                );
            } catch {
                // continue regardless of error
    ...
    extractProductVariants($, body) {
        ...
        if (productJsonMeta) {
            if (Object.keys(productJsonMeta.colorToAsin).length) {
                for (const item in productJsonMeta.colorToAsin) {
                    ...
                    const variant = {
                        ...
                        is_current_product:
                            productJsonMeta.colorToAsin[item].asin ===
                            this.asin,
                        price: "",
                    };
                    try {
                        const price = $(
                            `li[data-defaultasin="${productJsonMeta.colorToAsin[item].asin}"] div.twisterSlotDiv`
                        ).text();
            ...
            try {
                for (const item of variantsSelector[0].children) {
                    const variant = {
                        asin: "",
                        images: "",
                        title: "",
                    ...
                    if (item.attribs["data-dp-url"]) {
                        const findAsin = /dp\/(\w+)\//.exec(
                            item.attribs["data-dp-url"]
                        );
                        if (findAsin) {
                    ...
                    try {
                        const price = $(
                            `li[id=${item.attribs.id}] div.twisterSlotDiv`
                        ).text();
                        if (price) variant.price = price;
                        variant.images =
                    ...
                    variant.title = item.attribs.title
                        ? item.attribs.title.split(
                              this.geo.variants.split_text
                          )[1]
                        : ""; //item.children[0].children[0].children[0].children[0].children[0].children[0].children[1].children[0].children[0].attribs.alt;
    ...
    extractAlsoBought($) {
        ...
        for (const item of bought[0].children[0].children) {
            const product = {
                asin: "",
                title: "",
                image: [],
            ...
            try {
                const asin = JSON.parse(
                    item.children[0].attribs["data-p13n-asin-metadata"]
                );
            ...
            try {
                const review = $(
                    item.children[0].children[1].children[1]
                ).text();
                if (review) {
                    product.reviews.total_reviews = parseInt(
                        review.replace(/[^\d]/g, ""),
                        10
                    );
                }
            ...
            try {
                const rating =
                    item.children[0].children[1].children[0].children[0];
                if (rating) {
                    const classRating = rating.attribs.class;
                    if (classRating.includes("a-star-")) {
                        const splitRating = classRating.split("a-star-")[1];
                        product.reviews.rating = parseFloat(
                            splitRating.replace("-", ".")
                        );
                    }
                }
    ...
    extractSponsoredProducts($) {
        ...
        const sponsored = $(
            "#sp_detail-none > div:nth-child(3) > div > div > div.a-carousel-col.a-carousel-center > div"
        );
        ...
        for (const item of sponsored[0].children[0].children) {
            const product = {
                asin: "",
                title: "",
                image: [],
            ...
            const review = $(
                `#sp_detail-none_${product.asin} > div:nth-child(6) > a > span`
            ).text();
            if (review) {
                product.reviews.total_reviews = parseInt(
                    review.replace(/[^\d]/g, ""),
            ...
            const price = $(
                `#sp_detail-none_${product.asin} > div.a-row.a-color-price > a:nth-child(1) > span`
            ).text();
            if (price) {
                product.price.current_price = this.geo.price_format(price);
            }
            ...
            const prime = $(
                `#sp_detail-none_${product.asin} > div.a-row.a-color-price > a:nth-child(2) > span > i`
            );
            if (prime[0]?.attribs.class.includes("a-icon-prime")) {
                product.badges.amazon_prime = true;
            }

            const rating = $(
                `#sp_detail-none_${product.asin} > div:nth-child(6) > a > i`
            );
            if (rating[0]) {
                const classRating = rating[0].attribs.class;
                if (classRating.includes("a-star-")) {
                    const splitRating = classRating.split("a-star-")[1];
                    product.reviews.rating = splitRating.includes("-")
                        ? splitRating.replace("-", ".")
                        : splitRating;
                }
            }
    ...
    grapAsinDetails(body) {
        ...
        output.item_available = $("span.qa-availability-message").text()
            ? false
            : true;
        ...
        if (
            body.includes("bookDescription_feature_div") &&
            !output.description
        ) {
        ...
        [output.product_information, output.bestsellers_rank] =
            this.extractProductInfromation($);
        ...
        if (merchantInfo) {
            try {
                if (merchantInfo.includes("Ships from and sold by ")) {
                    const splitSoldBy = merchantInfo.split(
                        "Ships from and sold by "
                    )[1];
                    output.product_information.sold_by = splitSoldBy.slice(
                        0,
                        -1
                    );
                    output.product_information.fulfilled_by = splitSoldBy.slice(
                        0,
                        -1
                    );
                } else {
                    const splitSoldBy = merchantInfo
                        .split("Sold by ")[1]
                        .split(" and ");
                    output.product_information.sold_by = splitSoldBy[0];
                    output.product_information.fulfilled_by = splitSoldBy[1]
                        .split("Fulfilled by ")[1]
                        .slice(0, -1);
                }
            } catch {
                // continue regardless of error
        ...
        if (maxOrderQty) {
            try {
                output.product_information.qty_per_order = maxOrderQty[0]
                    ? maxOrderQty[0].children.length
                    : "na";
            } catch {
                // continue regardless of error
            }
        ...
        output.title =
            $(`span[id="productTitle"]`).text() || $(".qa-title-text").text();
        output.reviews.total_reviews = $(`span[id="acrCustomerReviewText"]`)
            .text()
            .split(/\s/)[0]
            ? parseInt(
            ...
            : 0;
        output.reviews.answered_questions = $(`a[id="askATFLink"]`).text()
            ? parseInt($(`a[id="askATFLink"]`).text(), 10)
            : 0;
        output.reviews.rating = $(
            `span.reviewCountTextLinkedHistogram.noUnderline`
        )[0]
            ? $($(`span.reviewCountTextLinkedHistogram.noUnderline`)[0])
                  .text()
                  .split(/\s/g)[0]
            : 0;

        output.price.current_price = $(`#priceblock_ourprice`)[0]
            ? this.geo.price_format(
                  $($(`#priceblock_ourprice`)[0].children[0]).text()
              )
            : 0;
        ...
        output.badges.amazon_сhoice = $(`div.ac-badge-wrapper`)[0]
            ? true
            : false;
        output.badges.amazon_prime = $(`span[id="priceBadging_feature_div"]`)[0]
            ? true
            : false;

        output.price.discounted = $(
        ...
        output.price.before_price = output.price.discounted
            ? this.geo.price_format(
                  $(`span.priceBlockStrikePriceString.a-text-strike`).text()
              )
            : output.price.current_price;

        if (output.price.discounted) {
            output.price.savings_amount = +(
                output.price.before_price - output.price.current_price
            ).toFixed(2);

            output.price.savings_percent = +(
                (100 / output.price.before_price) *
                output.price.savings_amount
            ).toFixed(2);
        }
        ...
        for (const key in scrapingResult) {
            try {
                ...
                const discountSearch = $(
                    `div[data-asin=${key}] span[data-a-strike="true"]`
                )[0];
                const ratingSearch = $(
                    `div[data-asin=${key}] .a-icon-star-small`
                )[0];
                const titleThumbnailSearch = $(
                    `div[data-asin=${key}] [data-image-source-density="1"]`
                )[0];
                const amazonChoice = $(
                    `div[data-asin=${key}] span[id="${key}-amazons-choice"]`
                ).text();
                const bestSeller = $(
                    `div[data-asin=${key}] span[id="${key}-best-seller"]`
                ).text();
                const amazonPrime = $(`div[data-asin=${key}] .s-prime`)[0];

                if (priceSearch) {
                    scrapingResult[key].price.current_price =
                        this.geo.price_format(
                            $(priceSearch.children[0]).text()
                        );
                }
                ...
                if (discountSearch) {
                    scrapingResult[key].price.before_price =
                        this.geo.price_format(
                            $(discountSearch.children[0]).text()
                        );
                ...
                if (ratingSearch) {
                    scrapingResult[key].reviews.rating = parseFloat(
                        ratingSearch.children[0].children[0].data
                    );

                    scrapingResult[key].reviews.total_reviews = parseInt(
                        ratingSearch.parent.parent.parent.next.attribs[
                            "aria-label"
                        ].replace(/\,/g, "")
                    );

                    scrapingResult[key].score = parseFloat(
                        scrapingResult[key].reviews.rating *
                            scrapingResult[key].reviews.total_reviews
                    ).toFixed(2);
                }

                if (titleThumbnailSearch) {
                    scrapingResult[key].title =
                        titleThumbnailSearch.attribs.alt;

                    scrapingResult[key].thumbnail =
                        titleThumbnailSearch.attribs.src;
                }
            } catch (err) {
                continue;
            }
    ...
    validateRating(item) {
        if (
            this.scrapeType === "products" &&
            item.reviews.rating >= this.minRating &&
            item.reviews.rating <= this.maxRating
        ) {
            return item;
        }
        if (
    ...
    extractAuthors($) {
        ...
        if (byAuthors?.children) {
            byAuthors.children.forEach((item, index) => {
                try {
                    if (
                        item &&
                        item.name === "span" &&
                        item.attribs.class.includes("author")
                    ) {
                        let url = "";
                        let author = "";
                        let role = $(
                            `#bylineInfo > span:nth-child(${index}) > span.contribution`
                        ).text();
                        if (role) {
                        ...
                        if (item.children[0].name === "a") {
                            const link = $(
                                `#bylineInfo > span:nth-child(${index}) > a`
                            )[0].attribs.href;
                            if (link != "#") {
                            ...
                            const link = $(
                                `#bylineInfo > span:nth-child(1) > span.a-declarative > a.a-link-normal.contributorNameID`
                            )[0].attribs.href;
                            url = `${this.mainHost}${link}`;
                            author = $(
                                `#bylineInfo > span:nth-child(1) > span.a-declarative > a.a-link-normal.contributorNameID`
    ...
    extractOtherSellers($) {
        ...
        let position = 0;
        if (moreBuyingOptions?.children) {
            moreBuyingOptions.children.forEach((item, index) => {
                try {
                    if (
                        item.attribs.class &&
                        item.attribs.class ===
                            "a-box mbc-offer-row pa_mbc_on_amazon_offer"
                    ) {
                        position++;
                        const price = $(
                            `#mbc > div:nth-child(${
                                index + 1
                            }) > div > span.a-declarative > div > div:nth-child(1)`
                        ).text();
                        const seller = $(
                            `#mbc-sold-by-${position} > span.a-size-small.mbcMerchantName`
                        ).text();
                        const url = `${this.mainHost}${
                            $(
                                `#mbc-buybutton-addtocart-${position}-announce`
                            )[0].attribs.href
                        }`;
                        other_sellers.push({
                            position,
    ...
    extractImages($, body) {
         ...
         *  Product types: books
         * */
        const imgRegex = /'imageGalleryData' : (.+),'centerColMargin'/.exec(
            body
        );
        if (imgRegex) {
            try {
        ...
        if (!images.length) {
            const imageData =
                $("#imgBlkFront")[0] || $("#ebooksImgBlkFront")[0];
            if (imageData) {
                ...
                if (imageIdregex) {
                    images.push(
                        `https://images-na.ssl-images-amazon.com/images/I/${imageIdregex[1]}.jpg`
                    );
                }
            }
        }
        ...
        if (!images.length) {
            ...
            for (const element of thumbnail) {
                try {
                    const url =
                        element.children[0].children[0].children[1].children[0]
                            .attribs.src;
                    if (!url.includes("x-locale/common")) {
    ...
    extractBookInSeries($) {
        ...
        if (bookSeriesName) {
            try {
                const url = $(
                    "div.shoveler-cell:nth-child(2) > a:nth-child(1)"
                )[0].attribs.href;
                const image = $(`img.product-image:nth-child(1)`)[0].attribs
                    .src;
                const series_name = $(
                    "div.shoveler-cell:nth-child(2) > a:nth-child(2)"
                ).text();
                series.push({
                ...
                try {
                    ...
                    const rating = $(
                        ...
                        ? $(
                              ...
                              .split(/\s/g)[0]
                        : 0;
                    reviews.total_reviews = parseInt(
                        total_reviews.replace(/[^\d]/g, ""),
                        10
                    );
    ...
    async extractCategories() {
        ...
        const categories = {};
        for (const select of categorySelect.children) {
